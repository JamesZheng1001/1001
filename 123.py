...
class MainWindow(QMainWindow):
    ...
    def forecast(self):
        ...
        import os
        import shutil
        if os.path.exists("pre_images"):
            shutil.rmtree("pre_images")
        os.mkdir("pre_images")
        for name in os.listdir(dirname):
            ...
            if len(classes) > 0:
                ...
                cv.imwrite(os.path.join("pre_images", name), frame)
                cv.waitKey(30)

            else:
                pass
        QMessageBox.about(self,"提示","预测结果已经保存在pre_images文件夹")
    def TestImage(self):
        frame = self.image.copy()
        classes, confidences, boxes = self.net.detect(frame, confThreshold=0.1, nmsThreshold=0.4)
        if len(classes) > 0:
